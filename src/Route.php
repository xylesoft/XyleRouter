...
class Route implements RouteInterface, RouteCuttingInterface, RouteStoppingInterface, RouteMethodsInterface, RouteDefaultsInterface, RouteWhereInterface, RouteNamingInterface {
	 ...
	 */
	public function __construct($routePattern, $name, RouteInterface $parent = null, PatternParserInterface $parser = null) {
	...
	private function parse($pattern) {
		...
		if (!$this->regexPattern) {
			...
			if ($this->getParent() instanceof RouteInterface) {
				...
				$tokens = array_merge($tokens, $this->getParent()->getTokens());
				$optionalTokens = array_merge($optionalTokens, $this->getParent()->getOptionalTokens());
			}

			$this->regexPattern = $this->parser->parse($pattern, $tokens, $optionalTokens);
		}
	...
	public function match(RequestInterface $request) {
		...
		$matchPattern = $this->parse($this->routePattern);
		if (preg_match_all($matchPattern, $request->getUrl(), $parameters)) {
